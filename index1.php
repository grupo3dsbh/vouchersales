...
// Carrega lista de meses disponíveis ANTES de usar
...
$godmode_data = [];
if ($godmode_enabled && !empty($available_months)) {
    // Processa todos os meses disponíveis
    foreach ($available_months as $month) {
        ...
        if (file_exists($month_file)) {
            ...
            if (($handle = fopen($month_file, 'r')) !== FALSE) {
                ...
                while (($row = fgetcsv($handle, 10000, $delimiter)) !== FALSE) {
                    if (count($headers) === count($row)) {
                        $month_data[] = array_combine($headers, $row);
                    }
                }
                fclose($handle);
            }
            
            // Processa vouchers do mês
            $month_vouchers = [];
            foreach ($month_data as $row) {
                ...
                if (!empty($voucher) && !empty($promoter)) {
                    ...
                    if (!isset($month_vouchers[$voucher])) {
                        $month_vouchers[$voucher] = [
                            'promoter' => $promoter,
                            'value' => 0
                        ];
                    }
                    $month_vouchers[$voucher]['value'] += $value;
                }
            }
    ...
    // Ordena consultores alfabeticamente
    ksort($godmode_data);
}
// ===== FIM NOVO =====
...
<body>
    <div class="container">
        <?php if (!$is_admin_mode): ?>
            ...
            <?php if ($godmode_enabled && !empty($godmode_data)): ?>
                        ...
                        <tbody>
                            <?php foreach ($godmode_data as $promoter_name => $promoter_data): ?>
                                <tr class="godmode-row" data-promoter="<?= strtolower(htmlspecialchars($promoter_name)) ?>">
                                    ...
                                    <td class="total-column">R$ <?= number_format($promoter_data['total_value'], 2, ',', '.') ?></td>
                ...
                function openDetailsModal(promoterName) {
                    ...
                    if (!promoterData) {
                        modalTableBody.innerHTML = '<tr><td colspan="4">Nenhum dado encontrado</td></tr>';
